...
do
  ...
  local _base_0 = {
    stringify = function(self)
      if self.passed then
        self.line[2][7] = ([[\c&H%s&]]):format(settings['bar-background'])
      else
        self.line[2][7] = ([[\c&H%s&]]):format(settings['bar-foreground'])
      end
    ...
    updateSize = function(self, w, h)
      ...
      self.y = h - settings['hover-zone-height']
      self.line[1][2] = ([[%d,%d]]):format(self.x + 10, h - settings['hover-zone-height'])
  ...
  local _class_0 = setmetatable({
    __init = function(self, animationQueue, title, position, w, h)
      ...
      self.x = math.floor(w * self.position)
      self.w = 20
      self.h = settings['hover-zone-height']
      local x = self.x > w / 2 and math.min(w - rightMargin, self.x + 10) or math.max(leftMargin, self.x + 10)
      self.line = {
        {
          ([[{\fn%s\bord2\fs%d\pos(]]):format(settings.font, settings['title-font-size']),
          ([[%d,%d]]):format(x, h - settings['hover-zone-height']),
          ([[)\c&H%s&\3c&H%s&\an2\alpha&H]]):format(settings['title-foreground'], settings['title-background']),
          [[FF]],
          ([[&}%s]]):format(title)
        },
        {
          [[{\an2\bord0\p1\pos(]],
          ([[%d,%d]]):format(self.x + 10, h + 2),
          [[)\fscx]],
          100,
          [[\fscy]],
          100,
          ([[\c&H%s&]]):format(settings['bar-foreground']),
          [[}m 0 0 l 1 0 1 2 0 2]]
        }
      }
      self.passed = false
