...
--[[
    mpv_thumbnail_script.lua 0.3.1 - commit a15bd35 (branch master)
    https://github.com/TheAMM/mpv_thumbnail_script
    Built on 2017-12-16 03:31:08
]]--
...
function display_thumbnail(pos, value, ass)
    ...
    if not ((duration == nil) or (value == nil)) then
        ...
        if thumb_path then
            ...
            -- Draw background
            ass:new_event()
            ass:pos(pos.x - ass_w/2, pos.y - ass_h - (overlay_y_offset * msy))
            ass:append("{\\rDefault\\blur5\\bord0\\1c&H000000\\3c&H000000}")
            ass:draw_start()
            ass:rect_cw(0, 0, ass_w, ass_h)
            ass:draw_stop()
        ...
        if (element.type == "slider") then
            ...
            if not (pos == nil) then
                ...
                elseif (slider_lo.stype == "knob") then
                    elem_ass:round_rect_cw(xp, (9*innerH/20) + foV,
                        elem_geo.w - foH, (11*innerH/20) + foV, math.ceil(xp) < elem_geo.w - foH and 1 or 0)
                    elem_ass:round_rect_cw(foH, (3*innerH/8) + foV,
                        xp, (5*innerH/8) + foV, xp > innerH and 2 or 0)
...
layouts["slimbox"] = function ()
    ...
    -- styles
    local styles = {
        box = "{\\rDefault\\blur1\\bord1\\1c&H000000\\3c&H000000}",
        timecodes = "{\\1c&HFFFFFF\\3c&H000000\\fs20\\bord2\\blur1}",
        tooltip = "{\\1c&HFFFFFF\\3c&H000000\\fs18\\bord2\\blur1}",
    }
