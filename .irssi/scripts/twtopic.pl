...
$VERSION = '1.02'; # Modified by ahoka
%IRSSI = (
...
"|  /toggle twtopic          |Toggle scrolling.     |\n".
...
sub get_topic {
    ...
    if($type eq "QUERY") {
        $text = "You are now talking to: ".$name;
        return $text;
    }
...
Irssi::settings_add_bool('tech_addon', 'twtopic', 1);
...
sub reload {
    if (!Irssi::settings_get_bool('twtopic')) {
        $start_pos = Irssi::settings_get_int('twtopic_size') - 1;
    }
